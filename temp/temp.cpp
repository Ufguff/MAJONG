...
vector<int> avl_tile;
auto rd = random_device {}; // дл€ рандомизации раскладки
...
void draw_pole(){       //отрисовывает фишки на поле, а также сколько осталось и сколько пар доступно
   clearviewport();
   char output[20];
   sprintf(output, "ќсталось фишек: %d", CON_TILES);
   outtextxy(400, 50, output);
   acc_avl();
   sprintf(output, "ќсталось ходов: %d", pairAVL);
...
bool is_season(TILE tile1, TILE tile2)  //      проверка сезонна€ ли фишка
{
